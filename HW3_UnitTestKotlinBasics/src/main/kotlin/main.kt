...
//  Find String ===============================================
	...
	missrememberedSongLyrics.forEach { line:String ->
		val lineNum = SearchString.search(trueColorLyrics, line)
		val isA:String = if(lineNum > 0) "is" else "is NOT"   //   <-- search function call
		println("\"" + line + "\" " +  isA + " a line in the song \"True Colors\" on line " + lineNum)
	}
...
//			println("searchR (first["+first+"], last["+last+"])")
			if (first > last) {
//				println("first["+first+"] > last["+last+"]")
				return -1
			}
			var mid = (last + first) / 2
			if (hayStack[mid].isEmpty()) {
				var left = mid - 1
				var right = mid + 1
				var breakWhile = false;
				while (!breakWhile) {
					if (left < first && right > last) {
//						println("left["+left+"] < first["+first+"] && right["+right+"] > last["+last+"]")
						return -1
					} else if (right <= last && !hayStack[right].isEmpty()) {
						println("mid = right")
						mid = right
						breakWhile=true
					} else if (left >= first && !hayStack[left].isEmpty()) {
						println("mid = left")
						mid = left
						breakWhile=true
					}
